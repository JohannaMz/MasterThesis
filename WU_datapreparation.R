...
# change projection to match all other layers
rehe_WU_HE <- st_transform(rehe_WU_HE, projection)


# plot data within germany
germany_sf <- read_sf("SDM/gadm36_DEU_shp/gadm36_DEU_1.shp")
# change projection to match all other layers
germany_sf <- st_transform(germany_sf, crs = projection)
...
#already extract BW shape for later
bawu_sf <- filter(germany_sf, NAME_1 == "Baden-Württemberg")
bawu_sf$VARNAME_1 <- "BW"


#plot again: looks better now. 
germany +
  tm_shape(rehe_WU_HE) +
  tm_symbols(size = 0.5)


### add ATKIS Strassen layer ###
...
### snap accident coordinates to roads ###
...
rehe_WU_HE <- rehe_WU_HE[!outside, ] #delete these observations from the final data
...
#check some descriptives: mean width decreases in size 
roads_HE %>% 
  st_drop_geometry() %>% 
  group_by(wdm) %>% 
...
#adding gemeindestraßen (mostly the small city roads!), as they are not included in the normal roads layer but be quite important for the density calculation!
...
# calculate mean of density in moving windows (100 and 200 meters) 
...
#### Environmental predictors: landcover layers ####
...
# use template raster to get the same extent, projection and resolution. be careful with values, needs method ngb!!
...
# calculate mean of density in a moving windows
